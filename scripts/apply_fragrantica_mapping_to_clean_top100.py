...
def main():
    ...
    for row in registry_rows:
        ...
        if code in by_code: raise SystemExit(f'Safety stop: duplicate verified registry code {code}')
        by_code[code]=row
    ...
    for expected_rank,source in enumerate(clean_rows,start=1):
        ...
        if not fid or not furl:
            unresolved.append({'rank':rank,'shobi_code':code,'field':'fragrantica_identity','reason':'Registry entry missing Fragrantica ID or URL'})
            continue
        fid=int(fid)
        ...
        mapped.append({'rank':rank,'shobi_code':code,'shobi_name':clean(source.get('shobi_name')),'shobi_url':clean(source.get('url')),'inspired_by_shobi':clean(source.get('inspired_by')),'perfume':clean(verified.get('perfume')) or clean(source.get('inspired_by')),'fragrantica_id':fid,'fragrantica_url':furl,'social_card_url':urls['social_card_url'],'image_url':urls['image_url'],'main_notes':notes,'main_note_evidence':evidence,'gender':gender,'season':season,'verification':{'shobi_ranking':True,'fragrantica_identity':True,'main_notes':bool(notes),'gender':bool(gender),'season':bool(season),'mapping_join_key':'shobi_code','identity_registry':'Fragrantica ID Database/mappings/bestseller-001-100.json'}})

    payload={'schema_version':1,'method':'Fragrantica ID Mapping Rule','built_at':datetime.now(timezone.utc).isoformat(),'ranking_authority':clean_payload.get('ranking_authority'),'identity_join_key':'shobi_code','identity_authority':'Fragrantica ID Database/mappings/bestseller-001-100.json','target_count':100,'mapped_count':len(mapped),'identity_verified_count':len(mapped),'main_notes_verified_count':sum(1 for x in mapped if x['verification']['main_notes']),'gender_verified_count':sum(1 for x in mapped if x['verification']['gender']),'season_verified_count':sum(1 for x in mapped if x['verification']['season']),'records':mapped}
