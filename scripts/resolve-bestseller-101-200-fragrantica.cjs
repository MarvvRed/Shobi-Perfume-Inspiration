...
(async()=>{
 ...
 const contexts=browser.contexts(); const ctx=contexts[0];
 if(!ctx) throw new Error('No Edge browser context available through CDP');
 ...
 for(const item of items){
  ...
  try{
   ...
   if(best && best.ratio>=0.5 && best.brandHit){resolved.push({...item,fragrantica_id:best.id,fragrantica_url:best.href,match_score:Number(best.ratio.toFixed(3)),resolver:'bing-self-hosted-isolated-page'}); console.log(`RESOLVED #${item.rank} ${item.code} id=${best.id} score=${best.ratio.toFixed(2)}`);}
