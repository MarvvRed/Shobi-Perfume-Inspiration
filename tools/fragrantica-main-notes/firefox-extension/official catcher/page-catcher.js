(() => {
  ...
  async function run() {
    ...
    if (!root) { emit('diagnostic', { stage: 'show-votes-missing-root', detail: 'Perfume pyramid root not found' }); return; }
    ...
    emit('diagnostic', { stage: 'top5-ready', detail: top5.map(n => `#${n.rank} ${n.note}=${n.votes}`).join(' | ') });
    emit('capture', { payload: { perfume: document.querySelector('h1')?.innerText?.trim() || document.title, url: location.href.split('#')[0], capture_method: 'show-votes-top5', captured_at: new Date().toISOString(), total_voted_notes: ranked.length, weights_sum: null, notes: top5 } });
  }
