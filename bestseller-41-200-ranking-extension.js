// Best Seller ranking bridge after CANONICAL-TOP100-v1 has loaded.
// IMPORTANT: ranks 1-100 are rebuilt ONLY from the frozen canonical payload.
// Legacy ranking may continue only for 101+ and must never influence Top100 order.
(function(){
  const canonicalRows=Object.values(window.SHOBI_FRAGRANTICA_CANONICAL_TOP100||{})
    .slice()
    .sort((a,b)=>Number(a.rank)-Number(b.rank));

  if(canonicalRows.length!==100){
    throw new Error(`CANONICAL-TOP100-v1 ranking isolation failed: expected 100 rows, got ${canonicalRows.length}`);
  }
  for(let i=0;i<100;i++){
    const expected=i+1;
    const actual=Number(canonicalRows[i].rank);
    const code=String(canonicalRows[i].shobi_code||'').trim();
    if(actual!==expected || !code){
      throw new Error(`CANONICAL-TOP100-v1 invalid ranking row at ${expected}: rank=${actual} code=${code}`);
    }
  }

  // Preserve historical globalRank metadata only as metadata; it never controls Top100 ordering.
  const oldRanking=window.SHOBI_BESTSELLER_RANKING||[];
  const oldGlobalByCode=new Map(oldRanking
    .filter(x=>x && x.code && x.globalRank!=null)
    .map(x=>[String(x.code).trim().toUpperCase(),x.globalRank]));

  const ranking=canonicalRows.map(row=>{
    const code=String(row.shobi_code).trim();
    return {
      rank:Number(row.rank),
      globalRank:oldGlobalByCode.get(code.toUpperCase())??null,
      code
    };
  });
  const seen=new Set(ranking.map(x=>Number(x.rank)));

  // Existing continuation 101-200 is outside CANONICAL-TOP100-v1 and remains unchanged.
  ...
  extra.forEach(x=>{
    ...
    if(!seen.has(r)){ranking.push(x);seen.add(r);}
  });
  ...
  window.SHOBI_CANONICAL_TOP100_RANKING_ISOLATED=true;
  console.log('CANONICAL-TOP100-v1 ranking isolated: ranks 1-100 rebuilt exclusively from canonical payload.');
})();
